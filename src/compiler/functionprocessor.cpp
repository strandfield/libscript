...
/*!
 * \fn void fill_prototype(FunctionBlueprint& blueprint, const std::shared_ptr<ast::FunctionDecl>& fundecl, const Scope& scp)
 * \brief fills the prototype field of a function blueprint
 * \param the function blueprint to fill
...
void fill_prototype(FunctionBlueprint& blueprint, const std::shared_ptr<ast::FunctionDecl>& fundecl, const Scope& scp)
{
  ...
  if (!fundecl->is<ast::ConstructorDecl>() && !fundecl->is<ast::DestructorDecl>())
    blueprint.prototype_.setReturnType(script::compiler::resolve_type(fundecl->returnType, scp));

  for (size_t i(0); i < fundecl->params.size(); ++i)
  {
    Type argtype = script::compiler::resolve_type(fundecl->params.at(i).type, scp);
    blueprint.prototype_.push(argtype);
  }
}
